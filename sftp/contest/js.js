...
if(!ar['blanket'] || ar['blanket'].length==0){} else { document.getElementById("blanket").style.display="block";document.getElementById("broke").style.display="block";document.getElementById(ar['blanket']).style.opacity="1";
}
if(!ar['update'] || ar['update'].length==0){} else { document.getElementById("def"+ar['update']).style.display="none";document.getElementById("abc"+ar['update']).style.display="block";
}
if(!ar['remaining_bal'] || ar['remaining_bal'].length==0){} else { document.getElementById("remaining_bal").innerHTML=ar['remaining_bal'];
}
...
//document.body.innerHTML+=ar['doc'];		// TO ADD DOc Bunch of DIVs
            ...
            alert('Internet Connection Error'); // An error occurred during the request.
